...
namespace PedestrianCrossingToolkit
{
    public class CrossingAppliedOverlay : MonoBehaviour
    {
        ...
        private static CrossingConnectivityLink[] LinkRenderBuffer = new CrossingConnectivityLink[512];
        private static CrossingPathWorkOrder[] PathWorkOrderRenderBuffer = new CrossingPathWorkOrder[1024];
        ...
        private const float CrossingDetailsPanelWidth = 286f;
        private const float CrossingDetailsPanelHeight = 126f;
        private const float CrossingDetailsBelowCrossingPixels = 124f;
        ...
        private int _highlightCachedRegistryRevision = -1;
        private PedestrianToolMode _worldVisualCachedMode = PedestrianToolMode.None;
        private CrossingPlacementRecord _routeCachedPreview = CrossingPlacementRecord.None;
        private Vector3 _worldVisualCachedCameraPosition;
        private Quaternion _worldVisualCachedCameraRotation;
        private float _worldVisualCachedCameraSize;
        ...
        private int _selectedCrossingAssetId;
        private CrossingPlacementAsset _selectedCrossingAsset = CrossingPlacementAsset.None;
        private int _selectedCrossingRegistryRevision = -1;
        ...
        public static void CreateIfNeeded(UIView view)
        {
            ...
            PedestrianCrossingLog.Advanced("[PedestrianCrossingToolkit] Applied overlay attached.");
        }
        ...
        public static void DestroyInstance()
        {
            ...
            Instance.ClearCrossingSelection();
        ...
        internal static bool SelectCrossingFromDefaultTool(CrossingPlacementAsset asset)
        {
            if (Instance == null || !PedestrianCrossingToolkitState.Enabled || asset.Id == 0)
                return false;

            Instance.SelectCrossing(asset);
            return Instance._selectedCrossingAssetId == asset.Id;
        }

        internal static void ClearCrossingSelectionFromDefaultTool()
        {
            if (Instance != null)
                Instance.ClearCrossingSelection();
        }

        internal static bool TryGetDefaultToolSelectedCrossing(
            out CrossingPlacementAsset asset)
        {
            asset = CrossingPlacementAsset.None;
            return Instance != null && Instance.TryGetSelectedCrossing(out asset);
        }
        ...
        private void UpdateCore()
        {
            ...
            UpdateCrossingSelectionLifecycle();
            ...
            bool cameraChanged = HasWorldVisualCameraChanged(camera);
            ...
            if (ShouldShowRouteWorldVisuals())
            {
                if (cameraChanged
                    ...
                    || !IsSameRoutePreview(_routeCachedPreview, currentPreview))
                {
                    RebuildRouteWorldVisuals();
                    _routeCachedPreview = currentPreview;
                }
            }
            else
            {
                ClearRouteWorldVisuals();
                _routeCachedPreview = CrossingPlacementRecord.None;
            }
            ...
            _worldVisualCachedMode = activeMode;
            _worldVisualCachedCrossingTabOpen = crossingTabOpen;
            _highlightCachedRegistryRevision = registryRevision;
            if (cameraChanged)
                CaptureWorldVisualCamera(camera);
        }

        private static void EnsureRenderBufferCapacity()
        {
            ManagerCapacity.EnsureArrayCapacity(ref LinkRenderBuffer, CrossingConnectivityPlanner.LinkCount);
            ManagerCapacity.EnsureArrayCapacity(ref PathWorkOrderRenderBuffer, CrossingPathWorkOrderPlanner.WorkOrderCount);
        ...
        private bool HasWorldVisualCameraChanged(Camera camera)
        {
            if (!_hasWorldVisualCameraSnapshot)
                return true;

            return (camera.transform.position - _worldVisualCachedCameraPosition).sqrMagnitude > 1f
                   || Quaternion.Angle(camera.transform.rotation, _worldVisualCachedCameraRotation) > 0.5f
                   || Mathf.Abs(GetOverlayCameraSize(camera) - _worldVisualCachedCameraSize) > 0.5f;
        }

        private void CaptureWorldVisualCamera(Camera camera)
        {
            _worldVisualCachedCameraPosition = camera.transform.position;
            _worldVisualCachedCameraRotation = camera.transform.rotation;
            _worldVisualCachedCameraSize = GetOverlayCameraSize(camera);
            _hasWorldVisualCameraSnapshot = true;
        }

        private static bool IsSameRoutePreview(CrossingPlacementRecord first, CrossingPlacementRecord second)
        {
            return first.Mode == second.Mode
                   && first.SegmentId == second.SegmentId
                   && first.SecondarySegmentId == second.SecondarySegmentId
                   && Mathf.Abs(first.SegmentPosition - second.SegmentPosition) < 0.001f
                   && Mathf.Abs(first.SecondarySegmentPosition - second.SecondarySegmentPosition) < 0.001f
                   && (first.WorldPosition - second.WorldPosition).sqrMagnitude < 0.01f
                   && (first.SecondaryWorldPosition - second.SecondaryWorldPosition).sqrMagnitude < 0.01f;
        }
        ...
        private static void DrawBulldozeCrossingHighlight(Camera camera)
        {
            CrossingPlacementAsset asset;
            if (!PedestrianCrossingBulldozeHarmony.TryGetHoveredAsset(out asset))
                return;

            Vector3 center = asset.Plan.IsValid
                ? asset.Plan.Center
            ...
            float x = screen.x - iconSize * 0.5f;
            float y = Screen.height - screen.y - iconSize * 0.5f;
            Rect border = new Rect(x - 4f, y - 4f, iconSize + 8f, iconSize + 8f);
            GUI.color = new Color(0.95f, 0.2f, 0.16f, 0.96f);
            ...
            Texture2D icon = GetCrossingLocationIconTexture(asset.Placement.Mode);
            if (icon != null)
            {
                GUI.color = Color.white;
                GUI.DrawTexture(new Rect(x + 5f, y + 5f, iconSize - 10f, iconSize - 10f), icon);
            }

            string text = "Remove PCT "
                          + PedestrianCrossingToolkitState.GetModeLabel(asset.Placement.Mode)
                          + "\nClick to remove this crossing only";
            Rect labelRect = new Rect(x + iconSize + 10f, y + 2f, 258f, 52f);
            GUI.color = new Color(0.06f, 0.07f, 0.08f, 0.94f);
            GUI.DrawTexture(labelRect, Texture2D.whiteTexture);
            GUI.color = Color.white;
            GUI.Label(new Rect(labelRect.x + 9f, labelRect.y + 7f, labelRect.width - 18f, labelRect.height - 12f), text);
        }
        ...
        {
            return asset.Id != 0 && asset.Plan.IsValid;
        }

        private static bool ShouldShowCrossingHighlights(Camera camera, PedestrianToolMode activeMode)
        {
            return PedestrianCrossingRoadsTab.IsOpen
                   && GetOverlayCameraSize(camera) >= CrossingHighlightMinCameraSize;
        }

        private void UpdateCrossingDetails(Camera camera)
        {
            CrossingPlacementAsset selectedAsset;
            if (!TryGetSelectedCrossing(out selectedAsset)
                || !selectedAsset.Plan.IsValid
                || !IsHighlightInCameraView(camera, selectedAsset)
            ...
            DrawCrossingDetails(camera, CrossingSummaryBlockingRects, view, selectedAsset);
        }

        private void UpdateCrossingSelectionLifecycle()
        {
            if (_selectedCrossingAssetId == 0)
                return;
            ...
            if (PedestrianCrossingToolkitState.ActiveMode != PedestrianToolMode.None
                ...
                || currentTool == null
                || currentTool.GetType() != typeof(DefaultTool))
            {
                ClearCrossingSelection();
                return;
            }
        ...
        private void SelectCrossing(CrossingPlacementAsset asset)
        {
            if (asset.Id == 0)
            {
                ClearCrossingSelection();
            ...
            bool changed = _selectedCrossingAssetId != asset.Id;
            _selectedCrossingAssetId = asset.Id;
            _selectedCrossingAsset = asset;
            _selectedCrossingRegistryRevision = CrossingPlacementRegistry.Revision;
            if (changed)
            {
                PedestrianCrossingLog.Advanced(
                    "[PedestrianCrossingToolkit] Crossing selected for summary: id="
                    + asset.Id
                    + " mode="
        ...
        private void ClearCrossingSelection()
        {
            _selectedCrossingAssetId = 0;
            _selectedCrossingAsset = CrossingPlacementAsset.None;
            _selectedCrossingRegistryRevision = -1;
        }

        private bool TryGetSelectedCrossing(out CrossingPlacementAsset asset)
        {
            asset = CrossingPlacementAsset.None;
            if (_selectedCrossingAssetId == 0)
                return false;
            ...
            int registryRevision = CrossingPlacementRegistry.Revision;
            if (_selectedCrossingRegistryRevision != registryRevision)
            {
                CrossingPlacementAsset refreshedAsset;
                if (!CrossingPlacementRegistry.TryGetAssetById(
                        _selectedCrossingAssetId,
                        out refreshedAsset))
                {
                    ClearCrossingSelection();
                    return false;
                ...
                _selectedCrossingAsset = refreshedAsset;
                _selectedCrossingRegistryRevision = registryRevision;
            }

            asset = _selectedCrossingAsset;
            return asset.Id != 0;
        }

        private void DrawCrossingDetails(
            Camera camera,
            List<Rect> blockingRects,
            UIView view,
            CrossingPlacementAsset asset)
        {
            Vector3 screen = camera.WorldToScreenPoint(asset.Plan.Center);
            if (screen.z <= 0f)
            {
                HideCrossingSummaryPanels();
                return;
            ...
            float x = Mathf.Clamp(
                screen.x - (CrossingDetailsPanelWidth * 0.5f),
                0f,
                Mathf.Max(0f, Screen.width - CrossingDetailsPanelWidth));
            float y = Mathf.Clamp(
                Screen.height - screen.y + CrossingDetailsBelowCrossingPixels,
                0f,
                Mathf.Max(0f, Screen.height - CrossingDetailsPanelHeight));
            Rect summaryRect = new Rect(x, y, CrossingDetailsPanelWidth, CrossingDetailsPanelHeight);
            ...
            EnsureCrossingSummaryPanelPool(view, 1);
            if (_crossingSummaryPanels.Count <= 0)
                return;

            _crossingSummaryPanels[0].Show(summaryRect, view, asset);
            HideCrossingSummaryPanelsFrom(1);
        }
        ...
        private static void CollectCrossingSummaryBlockingRect(UIComponent component, UIView view)
        {
            if (component == null
                || component == view
                || !component.enabled
                || !component.isVisible
                || component.opacity <= 0.05f
                || component.width < CrossingSummaryUiOcclusionMinWidth
                || component.height < CrossingSummaryUiOcclusionMinHeight)
            {
                return;
            }
            ...
            if (rect.width >= Screen.width - CrossingSummaryUiOcclusionPadding
                && rect.height >= Screen.height - CrossingSummaryUiOcclusionPadding)
            {
                return;
            }

            CrossingSummaryBlockingRects.Add(new Rect(
                rect.x - CrossingSummaryUiOcclusionPadding,
                rect.y - CrossingSummaryUiOcclusionPadding,
                rect.width + (CrossingSummaryUiOcclusionPadding * 2f),
                rect.height + (CrossingSummaryUiOcclusionPadding * 2f)));
        }
        ...
        private static bool OverlapsAny(Rect rect, List<Rect> blockingRects)
        {
            ...
            {
                if (rect.Overlaps(blockingRects[i]))
                    return true;
            }
        ...
        {
            return camera != null
                   && PedestrianCrossingRoadsTab.IsOpen
                   && PedestrianCrossingToolkitState.HasValidationProblemAssets;
        }
        ...
        private GameObject GetCrossingHighlightWorldVisual(string name, Material material)
        {
            ...
            _crossingHighlightWorldVisualActiveCount++;
            return visual;
        }
        ...
        private void AddRouteWorldVisual(Vector3 start, Vector3 end, float width)
        {
            ...
            if (renderer != null)
                renderer.sharedMaterial = GetRouteWorldMaterial();
        }
