...
{
    /// <summary>
    /// Retained selected-crossing summary. The component keeps one presentation alive
    /// and changes only its position or live state while that crossing remains selected.
    /// </summary>
    internal sealed class CrossingPassiveSummaryPanel : UIPanel
    {
            ...
            CrossingPathBuilder.SignalControllerDebugSnapshot snapshot)
        {
            if (hasSignal)
            {
                ...
                _labels[5].text = "Selected crossing.";
                return;
            }
