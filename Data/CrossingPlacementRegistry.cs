...
namespace PedestrianCrossingToolkit
{
    public static class CrossingPlacementRegistry
    {
        ...
        internal static bool TryGetAssetNearScreenOnDemand(
            Camera camera,
            Vector2 screenPosition,
            float pickRadiusPixels,
            out CrossingPlacementAsset asset)
        ...
        internal static bool HasAssetWithinHorizontalDistance(
            Vector3 center,
            float distance,
            int ignoredAssetId)
        {
            ...
            for (int i = 0; i < Assets.Count; i++)
            {
                ...
                if (asset.Id == 0 || asset.Id == ignoredAssetId)
                    continue;

                Vector3 assetCenter = asset.Plan.IsValid
                    ? asset.Plan.Center
                    : asset.Placement.WorldPosition;
                float dx = assetCenter.x - center.x;
                float dz = assetCenter.z - center.z;
                if ((dx * dx) + (dz * dz) < distanceSqr)
                    return true;
            }
        ...
        internal static bool IsAssetTouchingSegment(CrossingPlacementAsset asset, ushort segmentId)
        {
        ...
        public static int Restore(byte[] data)
        {
            ...
            using (BinaryReader reader = new BinaryReader(stream))
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    restoredAssets.Add(new CrossingPlacementAsset(id, placement, CrossingPlacementPlan.Invalid, signalRoadState));
                    if (id > maxId)
                        maxId = id;
                }
        ...
        private static SignalRoadStateSnapshot ReadSignalRoadStateSnapshot(BinaryReader reader)
        {
            ...
            if (segmentCount < 0 || segmentCount > MaxSignalRoadSegments)
                throw new InvalidDataException("Signal road snapshot segment count is invalid: " + segmentCount);
        ...
        public static int PruneDuplicateAssets()
        {
            ...
            int removed = 0;
            bool[] remove = new bool[Assets.Count];
            Dictionary<long, List<int>> newerAssetsByCell = new Dictionary<long, List<int>>();
            for (int i = Assets.Count - 1; i >= 0; i--)
            {
                CrossingPlacementAsset candidate = Assets[i];
                int cellX = Mathf.FloorToInt(candidate.Placement.WorldPosition.x / 16f);
                int cellZ = Mathf.FloorToInt(candidate.Placement.WorldPosition.z / 16f);
                bool duplicate = false;
                for (int dz = -1; dz <= 1 && !duplicate; dz++)
                {
                    for (int dx = -1; dx <= 1 && !duplicate; dx++)
                    {
                        List<int> newerIndices;
                        if (!newerAssetsByCell.TryGetValue(MakeRestoreCellKey(cellX + dx, cellZ + dz), out newerIndices))
                            continue;

                        for (int j = 0; j < newerIndices.Count; j++)
                        {
                            CrossingPlacementAsset newer = Assets[newerIndices[j]];
                            if (!IsSameAssetLocation(newer, candidate.Placement, candidate.Plan))
                                continue;
                ...
                if (!duplicate)
                    AddRestoreCellIndex(newerAssetsByCell, cellX, cellZ, i);
                if (!duplicate && candidate.Placement.HasSecondaryPoint)
                {
                    int secondaryCellX = Mathf.FloorToInt(candidate.Placement.SecondaryWorldPosition.x / 16f);
                    int secondaryCellZ = Mathf.FloorToInt(candidate.Placement.SecondaryWorldPosition.z / 16f);
                    AddRestoreCellIndex(newerAssetsByCell, secondaryCellX, secondaryCellZ, i);
                }
            }
        ...
        private static void AddRestoreCellIndex(Dictionary<long, List<int>> cells, int cellX, int cellZ, int assetIndex)
        {
            long key = MakeRestoreCellKey(cellX, cellZ);
            List<int> indices;
            if (!cells.TryGetValue(key, out indices))
            {
        ...
        private static long MakeRestoreCellKey(int cellX, int cellZ)
        {
