...
namespace PedestrianCrossingToolkit
{
    ...
    public static class RoadSnapResolver
    {
        ...
        private const int NetGridResolution = 270;
        private const float NetGridCellSize = 64f;
        private const float NetGridHalfExtent = 8640f;
        ...
        internal static bool TryResolveNearestLiveRoad(
            Vector3 worldPosition,
            float maxDistance,
            ...
            out Vector3 snappedPosition)
        {
            ...
            NetManager netManager = NetManager.instance;
            if (netManager == null || maxDistance <= 0f)
                return false;

            int minX = Mathf.Clamp(
                Mathf.FloorToInt((worldPosition.x - maxDistance + NetGridHalfExtent) / NetGridCellSize),
                0,
                NetGridResolution - 1);
            ...
            int minZ = Mathf.Clamp(
                Mathf.FloorToInt((worldPosition.z - maxDistance + NetGridHalfExtent) / NetGridCellSize),
                0,
                NetGridResolution - 1);
            ...
            for (int z = minZ; z <= maxZ; z++)
            {
                ...
                {
                    ushort candidateId = netManager.m_segmentGrid[(z * NetGridResolution) + x];
                    int safety = 0;
                    while (candidateId != 0 && safety < netManager.m_segments.m_size)
                    {
                        ref NetSegment candidate = ref netManager.m_segments.m_buffer[candidateId];
                        ushort nextId = candidate.m_nextGridSegment;
                        if ((candidate.m_flags & NetSegment.Flags.Created) != 0
                            && candidate.Info != null
                            && candidate.Info.m_netAI is RoadBaseAI)
                        {
                            float candidatePosition = EstimateSegmentPosition(
                                netManager,
                                candidateId,
                                worldPosition,
                                0.5f);
                            Vector3 candidatePoint = GetSegmentBezier(
                                GetNodePosition(netManager, candidate.m_startNode),
                                candidate.m_startDirection,
                                GetNodePosition(netManager, candidate.m_endNode),
                                candidate.m_endDirection).Position(candidatePosition);
                            float dx = candidatePoint.x - worldPosition.x;
                            float dz = candidatePoint.z - worldPosition.z;
                            float distanceSqr = (dx * dx) + (dz * dz);
                            if (distanceSqr < bestDistanceSqr)
                            {
                                bestDistanceSqr = distanceSqr;
                                segmentId = candidateId;
                                segmentPosition = candidatePosition;
                                snappedPosition = candidatePoint;
                            }
                        }
