...
namespace PedestrianCrossingToolkit
{
    ...
    public static class RoadPlacementRules
    {
        ...
        private static int _vanillaCrossingCacheRefreshNextSegmentId;
        ...
        public static void RequestVanillaCrossingCacheRefresh(string reason)
        {
            NetManager netManager = NetManager.instance;
            if (netManager == null)
            {
            ...
            StartVanillaCrossingCacheRefresh(reason);
        }
        ...
        public static void UpdateVanillaCrossingCache(float realTimeDelta)
        {
            ...
            if (!_hasVanillaCrossingNetworkSignature)
            {
                ...
                _hasVanillaCrossingNetworkSignature = true;
                StartVanillaCrossingCacheRefresh("initial");
                return;
            }
        ...
        private static void ProcessVanillaCrossingCacheRefresh(NetManager netManager)
        {
            ...
            int segmentLimit = ManagerCapacity.GetExclusiveUpperBound(
                netManager.m_segments.m_size,
                netManager.m_segments.m_buffer.Length);
            int endSegmentId = Mathf.Min(
                segmentLimit,
                _vanillaCrossingCacheRefreshNextSegmentId + VanillaCrossingCacheRefreshSegmentBatchSize);

            for (int segmentIndex = _vanillaCrossingCacheRefreshNextSegmentId; segmentIndex < endSegmentId; segmentIndex++)
                AddVanillaCrossingCacheEntriesForSegment(netManager, (ushort)segmentIndex, VanillaCrossingCacheRefreshBuffer);

            _vanillaCrossingCacheRefreshNextSegmentId = endSegmentId;
            if (_vanillaCrossingCacheRefreshNextSegmentId < segmentLimit)
                return;
        ...
        public static bool TryGetJunctionApproachCrossingPoint(
            ushort segmentId,
            bool endSegment,
            out VanillaCrossingPoint crossingPoint)
        {
            ...
            if (!IsThreePlusJunctionNode(nodeId))
                return false;

            Vector3 crossingPosition;
            if (!TryCalculateVanillaCrossingPosition(
                segmentId,
                ref segment,
                endSegment,
                out crossingPosition))
            {
                return false;
            }

            Vector3 nodePosition = netManager.m_nodes.m_buffer[nodeId].m_position;
            float segmentPosition = EstimateSegmentPosition(
                netManager,
                ref segment,
                crossingPosition);
            crossingPoint = new VanillaCrossingPoint(
                nodeId,
                endSegment,
                crossingPosition,
                segmentPosition,
                HorizontalDistance(nodePosition, crossingPosition));
            return true;
        }
