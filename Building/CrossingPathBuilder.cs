...
namespace PedestrianCrossingToolkit
{
    public static partial class CrossingPathBuilder
    {
        ...
        private const float BridgeWindowFrameWidth = 0.055f;
        private const float BridgeRoofThickness = 0.14f;
        private const float BridgeRoofOverhang = 0.12f;
        private const float BridgeRoofPitchRise = 0.22f;
        ...
        private const float BridgeDeckPanelSpacing = 2.2f;
        private const float BridgeDeckPanelJointWidth = 0.045f;
        private const float BridgeDeckPanelJointHeight = 0.025f;
        private const float BridgeDeckSideWarningWidth = 0.06f;
        private const float BridgeAccessHandrailHeight = 1.05f;
        private const float BridgeAccessHandrailThickness = 0.055f;
        private const float BridgeAccessHandrailInset = 0.18f;
        private const float BridgeAccessHandrailPostSpacing = 1.2f;
        ...
        private const float SubwayEntranceCanopyLength = 1.1f;
        private const float SubwayEntranceCanopyHeight = 1.35f;
        private const float SubwayEntranceCanopyRoofThickness = 0.08f;
        private const float SubwayEntranceCanopyOverhang = 0.18f;
        private const float SubwayEntranceCanopyPostWidth = 0.07f;
        private const float SubwayEntranceCanopyPitchRise = 0.14f;
        private const float SubwayEntranceTileBandHeight = 0.16f;
        private const float SubwayEntranceTileBandThickness = 0.018f;
        private const float SubwayEntranceTileJointSpacing = 0.36f;
        private const float SubwayEntranceTileJointWidth = 0.012f;
        private const float SubwayEntranceSignWidth = 0.72f;
        private const float SubwayEntranceSignHeight = 0.22f;
        private const float SubwayEntranceSignThickness = 0.035f;
        private const float SubwayEntranceHandrailHeight = 0.46f;
        private const float SubwayEntranceHandrailThickness = 0.035f;
        private const float SubwayEntranceHandrailPostSpacing = 0.45f;
        ...
        private const int CrossingStripeTextureSize = 96;
        ...
        private const int SignalPedestrianScansPerUpdate = 2;
        private const int SignalControllerReassertionsPerSimulationFrame = 4;
        private const float SignalControllerSchedulerLogSeconds = 10f;
        private const float SignalNearbyRoadSegmentSearchRadius = 18f;
        private const float SignalNearbyRoadSegmentGridSearchRadius = 256f;
        private const int SignalNetGridResolution = 270;
        private const float SignalNetGridCellSize = 64f;
        private const float SignalNetGridHalfExtent = 8640f;
        ...
        private struct DeferredNetworkRelease
        {
        ...
        private static bool _batchBuildActive;
        private static bool _unityVisualLifecycleEnabled = true;
        private static int _signalPedestrianScanCursor;
        private static int _signalControllerReassertCursor;
        private static float _signalControllerSchedulerLogElapsed;
        private static int _signalControllerSchedulerScans;
        private static int _signalControllerSchedulerDeferred;
        ...
        internal static bool TryValidateRoadReplacementNetworkAsset(int assetId, out string error)
        {
            ...
            {
                error = "restored registry asset is missing";
                return false;
            }

            if (!asset.Plan.IsValid)
            {
                error = "restored placement plan is invalid";
                return false;
            }

            int builtSegments = CountBuiltSegmentsForAsset(assetId);
            if (asset.Placement.Mode == PedestrianToolMode.SignalCrossing)
            {
                ...
                error = "restored signal controller is missing";
                return false;
            }

            if (builtSegments <= 0)
            {
                error = "restored crossing owns no executable path segment";
                return false;
            }

            bool gradeSeparated =
                asset.Placement.Mode == PedestrianToolMode.SubwayLink ||
                asset.Placement.Mode == PedestrianToolMode.SubwayPointToPoint ||
        ...
        internal static bool TryValidateRoadReplacementVisualAsset(int assetId, out string error)
        {
            error = string.Empty;
            CrossingPlacementAsset asset;
            if (!CrossingPlacementRegistry.TryGetAssetById(assetId, out asset))
            {
                error = "restored registry asset is missing during visual validation";
                return false;
            }
            ...
            switch (asset.Placement.Mode)
            {
                ...
                case PedestrianToolMode.SignalCrossing:
                    ...
                    foreach (string key in BuiltSurfaceVisualKeys)
                    {
                        if (key != null && key.StartsWith(surfacePrefix, StringComparison.Ordinal))
                            return true;
                    }
                    error = "restored surface-crossing visuals are missing";
                    return false;
                ...
                case PedestrianToolMode.SubwayPointToPoint:
                    ...
                    error = "restored subway-entrance visuals are missing";
                    return false;
                ...
                case PedestrianToolMode.PedestrianBridge:
                    ...
                    error = "restored bridge visuals are missing";
                    return false;

                default:
                    error = "restored crossing type has no visual validation contract";
                    return false;
            }
        }

        private static int CountBuiltSegmentsForAsset(int assetId)
        {
            int count = 0;
            foreach (KeyValuePair<ushort, List<int>> entry in BuiltSegmentAssets)
            {
        ...
        internal static void BeginNetworkOnlyBuild()
        {
            _unityVisualLifecycleEnabled = false;
        }

        internal static void EndNetworkOnlyBuild()
        {
            _unityVisualLifecycleEnabled = true;
        }

        internal static void RemoveUnityVisualsForAssets(int[] assetIds, int assetCount)
        {
            if (assetIds == null || assetCount <= 0)
                return;

            int max = Mathf.Min(assetCount, assetIds.Length);
            for (int i = 0; i < max; i++)
            {
                if (assetIds[i] > 0)
                    ClearBuiltVisualObjectsForAsset(assetIds[i]);
            }
        }

        internal static int RebuildUnityVisualsForAssets(int[] assetIds, int assetCount)
        {
            if (assetIds == null || assetCount <= 0)
                return 0;
            ...
            for (int i = 0; i < pathOrderMax; i++)
            {
                ...
                if (!ContainsAssetId(assetIds, assetCount, order.AssetId))
                    continue;
                    ...
                    order.SecondBuildPosition))
                {
                    AddBridgeConcrete(
                        order.AssetId,
                        GetDeckOverhangStart(order.FirstPosition, order.SecondPosition),
                        GetDeckOverhangEnd(order.FirstPosition, order.SecondPosition),
                        BridgeDeckVisualWidth,
                        CrossingVerticalProfile.BridgeDeckHeight,
                        CrossingVerticalProfile.BridgeDeckHeight,
                        BridgeConcreteThickness,
                        "deck");
            ...
            for (int i = 0; i < maxAssetCount; i++)
            {
                ...
                if (assetIds[i] > 0 &&
                    TryValidateRoadReplacementVisualAsset(assetIds[i], out validationError))
                {
                    validatedAssets++;
                }
            }
        ...
        private static bool ContainsAssetId(int[] assetIds, int assetCount, int assetId)
        {
            int max = Mathf.Min(assetCount, assetIds.Length);
            for (int i = 0; i < max; i++)
            {
        ...
        public static void BeginBuildBatch(bool clearExistingVisuals)
        {
            if (_batchBuildActive)
                return;

            _batchBuildActive = true;
            PendingSignalControlOrders.Clear();
            if (clearExistingVisuals)
                ClearBuiltVisualObjects();

            ApplySuppressedSurfaceCrossingFlags(true);
            RefreshManagedSegmentIndex();
        }

        public static void EndBuildBatch()
        {
            if (!_batchBuildActive)
                return;
        ...
        {
            ClearDeferredNetworkReleases(reason);
        ...
        public static int RemoveBuiltPathsForAsset(int assetId, string reason)
        {
            ...
            NetManager netManager = NetManager.instance;
            if (netManager == null || SimulationManager.instance == null)
                return 0;
            ...
            for (int i = BuiltSegments.Count - 1; i >= 0; i--)
            {
                ...
                RemoveManagedSegmentFromIndex(segmentId);
                if (segmentId != 0 && segmentId < netManager.m_segments.m_size)
                {
                    ref NetSegment segment = ref netManager.m_segments.m_buffer[segmentId];
                    if ((segment.m_flags & NetSegment.Flags.Created) != 0)
                        QueueDeferredNetworkRelease(segmentId, reason);
                }
                queued++;
            }

            PedestrianCrossingLog.Advanced("[PedestrianCrossingToolkit] Detached built paths for deferred native release: reason="
                      + reason
                      + " asset="
                      + assetId
                      + " queued="
        ...
        public static void ProcessDeferredNetworkRelease()
        {
            ...
            try
            {
                ...
                ref NetSegment segment = ref netManager.m_segments.m_buffer[release.SegmentId];
                if ((segment.m_flags & NetSegment.Flags.Created) == 0)
                    return;
        ...
        private static void ClearDeferredNetworkReleases(string reason)
        {
            int cleared;
            lock (DeferredNetworkReleaseGate)
            {
            ...
            if (cleared > 0)
            {
                PedestrianCrossingLog.Advanced("[PedestrianCrossingToolkit] Deferred managed path releases forgotten without native mutation: reason="
                          + reason
                          + " count="
                          + cleared);
            }
        ...
        private static void RemoveManagedSegmentFromIndex(ushort segmentId)
        {
            foreach (KeyValuePair<NetInfo, List<ushort>> entry in ManagedSegmentsByPrefab)
            {
                if (entry.Value != null)
                    entry.Value.Remove(segmentId);
            }
        }
        ...
        private static bool TryFindMatchingManagedSegment(NetInfo prefab, Vector3 expectedStart, Vector3 expectedEnd, out ushort matchingSegmentId)
        {
            ...
            NetManager netManager = NetManager.instance;
            List<ushort> candidates;
            if (!ManagedSegmentsByPrefab.TryGetValue(prefab, out candidates))
                return false;

            for (int candidateIndex = 0; candidateIndex < candidates.Count; candidateIndex++)
            {
                ushort segmentId = candidates[candidateIndex];
                ref NetSegment segment = ref netManager.m_segments.m_buffer[segmentId];
        ...
        private static void RefreshManagedSegmentIndex()
        {
            ManagedSegmentsByPrefab.Clear();
            VehicleRoadSegments.Clear();
            HashSet<ushort> deferredReleaseIds = null;
            lock (DeferredNetworkReleaseGate)
            {
                if (DeferredNetworkReleaseIds.Count > 0)
                    deferredReleaseIds = new HashSet<ushort>(DeferredNetworkReleaseIds);
            }
            ...
            for (int segmentIndex = 1; segmentIndex < segmentLimit; segmentIndex++)
            {
                ushort segmentId = (ushort)segmentIndex;
                if (deferredReleaseIds != null && deferredReleaseIds.Contains(segmentId))
                    continue;
                ...
                if (HasVehicleLane(segment.Info))
                    VehicleRoadSegments.Add(segmentId);
                if (IsManagedPathPrefab(segment.Info))
                    AddManagedSegmentToIndex(segment.Info, segmentId);
            }
        }

        private static void AddManagedSegmentToIndex(NetInfo prefab, ushort segmentId)
        {
            if (prefab == null || segmentId == 0)
                return;

            List<ushort> segments;
            if (!ManagedSegmentsByPrefab.TryGetValue(prefab, out segments))
            {
                segments = new List<ushort>();
                ManagedSegmentsByPrefab[prefab] = segments;
            }
        ...
        private static void ReleaseStaleLandingConnector(CrossingLandingConnectorWorkOrder order, Vector3 connectorFrom, Vector3 connectorTo)
        {
            ...
            NetManager netManager = NetManager.instance;
            List<ushort> candidates;
            if (!ManagedSegmentsByPrefab.TryGetValue(order.Prefab, out candidates))
                return;
        ...
        {
            if (!_unityVisualLifecycleEnabled)
                return;
        ...
        private static void AddBridgeAccessVisual(CrossingLandingAccessAssetWorkOrder order)
        {
            ...
            AddBridgeAccessHandrails(order.AssetId, deckBase, footPath, direction);
        }
        ...
        private static void AddBridgeAccessStairs(int assetId, Vector3 deckBase, Vector3 foot)
        {
            ...
            for (int i = 0; i < steps; i++)
            {
                ...
                if (i % BridgeAccessStepMarkerInterval == 0 || i == steps - 1)
                {
                    AddBridgeShellCube(
                        "PCT Bridge Stair tread marker #" + assetId,
                        center + Vector3.up * ((BridgeAccessStepThickness + BridgeAccessStepMarkerHeight) * 0.5f),
                        rotation,
                        new Vector3(BridgeAccessVisualWidth - 0.16f, BridgeAccessStepMarkerHeight, BridgeAccessStepMarkerDepth),
                        GetBridgeWarningStripeMaterial());
                }
            }
        }
        ...
        private static void AddBridgeAccessHandrails(int assetId, Vector3 deckBase, Vector3 foot, Vector3 direction)
        {
            Vector3 flatDirection = direction;
            flatDirection.y = 0f;
            if (flatDirection.sqrMagnitude <= 0.01f)
            ...
            Vector3 slopeDirection = span / length;
            Vector3 side = new Vector3(-flatDirection.z, 0f, flatDirection.x);
            float sideOffset = Mathf.Max(0.05f, BridgeAccessVisualWidth * 0.5f - BridgeAccessHandrailInset);
            Quaternion railRotation = Quaternion.LookRotation(slopeDirection, Vector3.up);

            AddBridgeShellCube(
                "PCT Bridge Metal access handrail L #" + assetId,
                (deckBase + foot) * 0.5f + side * sideOffset + Vector3.up * (BridgeConcreteThickness + BridgeAccessHandrailHeight),
                railRotation,
                new Vector3(BridgeAccessHandrailThickness, BridgeAccessHandrailThickness, length),
                GetBridgeTrimMaterial());
            AddBridgeShellCube(
                "PCT Bridge Metal access handrail R #" + assetId,
                (deckBase + foot) * 0.5f - side * sideOffset + Vector3.up * (BridgeConcreteThickness + BridgeAccessHandrailHeight),
                railRotation,
                new Vector3(BridgeAccessHandrailThickness, BridgeAccessHandrailThickness, length),
                GetBridgeTrimMaterial());

            int posts = Mathf.Max(2, Mathf.CeilToInt(length / BridgeAccessHandrailPostSpacing) + 1);
            for (int i = 0; i < posts; i++)
            {
                float t = posts == 1 ? 0.5f : i / (float)(posts - 1);
                Vector3 baseCenter = Vector3.Lerp(foot, deckBase, t) + Vector3.up * BridgeConcreteThickness;
                AddBridgeHandrailPost(assetId, baseCenter + side * sideOffset);
                AddBridgeHandrailPost(assetId, baseCenter - side * sideOffset);
            }
        }

        private static void AddBridgeHandrailPost(int assetId, Vector3 baseCenter)
        {
            AddBridgeShellCube(
                "PCT Bridge Metal access handrail post #" + assetId,
                baseCenter + Vector3.up * (BridgeAccessHandrailHeight * 0.5f),
                Quaternion.identity,
                new Vector3(BridgeAccessHandrailThickness, BridgeAccessHandrailHeight, BridgeAccessHandrailThickness),
                GetBridgeTrimMaterial());
        }
        ...
        private static void AddSubwayEntranceTileBands(int assetId, Vector3 position, SubwayEntranceTerrainFrame frame, float width, float length)
        {
            float sideOffset = width * 0.5f + SubwayEntranceTileBandThickness * 0.5f + 0.005f;
            Vector3 boxMid = position + frame.Forward * (length * 0.5f);
            Vector3 scale = new Vector3(SubwayEntranceTileBandThickness, SubwayEntranceTileBandHeight, Mathf.Max(0.1f, length - SubwayEntranceThresholdDepth * 2f));
            float tileHeightCenter = Mathf.Min(
                SubwayEntranceWallHeight - SubwayEntranceTileBandHeight * 0.5f,
                SubwayEntranceCopingHeight + SubwayEntranceTileBandHeight * 0.6f);

            AddSubwayEntranceBlock(
                assetId,
                frame.SurfacePoint(boxMid + frame.Side * sideOffset, 0f) + frame.Normal * tileHeightCenter,
                frame.Rotation,
                scale,
                "tile-band-left",
                GetSubwayEntranceTileMaterial());
            AddSubwayEntranceBlock(
                assetId,
                frame.SurfacePoint(boxMid - frame.Side * sideOffset, 0f) + frame.Normal * tileHeightCenter,
                frame.Rotation,
            ...
            float usableLength = Mathf.Max(0.1f, length - SubwayEntranceThresholdDepth * 2f);
            int joints = Mathf.FloorToInt(usableLength / SubwayEntranceTileJointSpacing);
            for (int i = 1; i <= joints; i++)
            {
                float distance = SubwayEntranceThresholdDepth + i * SubwayEntranceTileJointSpacing;
                if (distance >= length - SubwayEntranceThresholdDepth * 0.5f)
                    continue;

                AddSubwayTileJoint(assetId, position + frame.Forward * distance + frame.Side * sideOffset, frame, tileHeightCenter);
                AddSubwayTileJoint(assetId, position + frame.Forward * distance - frame.Side * sideOffset, frame, tileHeightCenter);
            }
        }

        private static void AddSubwayTileJoint(int assetId, Vector3 basePosition, SubwayEntranceTerrainFrame frame, float tileHeightCenter)
        {
            AddSubwayEntranceBlock(
                assetId,
                frame.SurfacePoint(basePosition, 0f) + frame.Normal * tileHeightCenter,
                frame.Rotation,
                new Vector3(SubwayEntranceTileBandThickness * 1.4f, SubwayEntranceTileBandHeight, SubwayEntranceTileJointWidth),
                "tile-joint",
                GetSubwayOpeningMaterial());
        }

        private static void AddSubwayEntranceStairRails(int assetId, Vector3 position, SubwayEntranceTerrainFrame frame, float width, float length)
        {
            float railLength = Mathf.Clamp(SubwayEntranceStepPanelLength - 0.18f, 0.4f, length - SubwayEntranceBackWallThickness - 0.22f);
            if (railLength <= 0.1f)
                return;

            Vector3 start = position + frame.Forward * 0.08f;
            Vector3 end = position + frame.Forward * railLength;
            float sideOffset = Mathf.Max(0.05f, width * 0.5f - SubwayEntranceHandrailThickness * 1.5f);
            AddSubwayEntranceRailRun(assetId, start, end, frame, sideOffset);
            AddSubwayEntranceRailRun(assetId, start, end, frame, -sideOffset);
        }

        private static void AddSubwayEntranceRailRun(int assetId, Vector3 start, Vector3 end, SubwayEntranceTerrainFrame frame, float sideOffset)
        {
            Vector3 run = end - start;
            if (run.sqrMagnitude <= 0.01f)
                return;

            Vector3 center = (start + end) * 0.5f + frame.Side * sideOffset;
            AddSubwayEntranceBlock(
                assetId,
                frame.SurfacePoint(center, SubwayEntranceSlopeSurfaceLift) + frame.Normal * SubwayEntranceHandrailHeight,
                frame.Rotation,
                new Vector3(SubwayEntranceHandrailThickness, SubwayEntranceHandrailThickness, run.magnitude),
                "stair-handrail",
                GetSubwayEntranceCanopyMaterial());

            int posts = Mathf.Max(2, Mathf.CeilToInt(run.magnitude / SubwayEntranceHandrailPostSpacing) + 1);
            for (int i = 0; i < posts; i++)
            {
                float t = posts == 1 ? 0.5f : i / (float)(posts - 1);
                Vector3 postBase = Vector3.Lerp(start, end, t) + frame.Side * sideOffset;
                AddSubwayEntranceBlock(
                    assetId,
                    frame.SurfacePoint(postBase, SubwayEntranceSlopeSurfaceLift) + frame.Normal * (SubwayEntranceHandrailHeight * 0.5f),
                    frame.Rotation,
                    new Vector3(SubwayEntranceHandrailThickness, SubwayEntranceHandrailHeight, SubwayEntranceHandrailThickness),
                    "stair-handrail-post",
                    GetSubwayEntranceCanopyMaterial());
            }
        }

        private static void AddSubwayEntranceCanopy(int assetId, Vector3 position, SubwayEntranceTerrainFrame frame, float width)
        {
            float canopyLength = Mathf.Max(0.4f, SubwayEntranceCanopyLength);
            float canopyWidth = width + SubwayEntranceWallThickness * 2f + SubwayEntranceCanopyOverhang * 2f;
            float halfWidth = canopyWidth * 0.5f - SubwayEntranceCanopyPostWidth * 0.5f;

            AddSubwayEntranceCanopyRoof(assetId, position, frame, canopyWidth, canopyLength);

            float postHeight = Mathf.Max(0.2f, SubwayEntranceCanopyHeight);
            AddSubwayEntranceCanopyPost(assetId, position + frame.Forward * (canopyLength * 0.18f) + frame.Side * halfWidth, frame, postHeight);
            AddSubwayEntranceCanopyPost(assetId, position + frame.Forward * (canopyLength * 0.18f) - frame.Side * halfWidth, frame, postHeight);
            AddSubwayEntranceCanopyPost(assetId, position + frame.Forward * (canopyLength * 0.86f) + frame.Side * halfWidth, frame, postHeight);
            AddSubwayEntranceCanopyPost(assetId, position + frame.Forward * (canopyLength * 0.86f) - frame.Side * halfWidth, frame, postHeight);

            AddSubwayEntranceBlock(
                assetId,
                frame.SurfacePoint(position + frame.Forward * (SubwayEntranceSignThickness * 0.5f), 0f)
                + frame.Normal * (SubwayEntranceCanopyHeight - SubwayEntranceSignHeight * 0.5f),
                frame.Rotation,
                new Vector3(SubwayEntranceSignWidth, SubwayEntranceSignHeight, SubwayEntranceSignThickness),
                "sign-panel",
                GetSubwayEntranceSignMaterial());
        }

        private static void AddSubwayEntranceCanopyRoof(int assetId, Vector3 position, SubwayEntranceTerrainFrame frame, float width, float length)
        {
            float halfWidth = width * 0.5f;
            Vector3 start = frame.SurfacePoint(position, 0f) + frame.Normal * SubwayEntranceCanopyHeight;
            Vector3 end = frame.SurfacePoint(position + frame.Forward * length, 0f) + frame.Normal * SubwayEntranceCanopyHeight;
            Vector3 side = frame.Side;
            Vector3 normal = frame.Normal;
            GameObject roof = new GameObject("PCT Subway Entrance pitched canopy #" + assetId);
            Mesh mesh = new Mesh();
            mesh.vertices = new[]
            {
                start - side * halfWidth + normal * (SubwayEntranceCanopyRoofThickness * 0.5f),
                start + side * halfWidth + normal * (SubwayEntranceCanopyRoofThickness * 0.5f),
                start + normal * (SubwayEntranceCanopyRoofThickness + SubwayEntranceCanopyPitchRise),
                end - side * halfWidth + normal * (SubwayEntranceCanopyRoofThickness * 0.5f),
                end + side * halfWidth + normal * (SubwayEntranceCanopyRoofThickness * 0.5f),
                end + normal * (SubwayEntranceCanopyRoofThickness + SubwayEntranceCanopyPitchRise)
            };
            mesh.triangles = new[]
            {
                0, 3, 5, 0, 5, 2,
        ...
        private static void AddSubwayEntranceCanopyPost(int assetId, Vector3 basePosition, SubwayEntranceTerrainFrame frame, float height)
        {
            AddSubwayEntranceBlock(
                assetId,
                frame.SurfacePoint(basePosition, 0f) + frame.Normal * (height * 0.5f),
                frame.Rotation,
                new Vector3(SubwayEntranceCanopyPostWidth, height, SubwayEntranceCanopyPostWidth),
                "canopy-post",
                GetSubwayEntranceCanopyMaterial());
        }
        ...
        private static bool ShouldUseVanillaSignalNodeCrossing(CrossingPlacementAsset asset)
        {
            ...
            // The join node is the persistent identity of a signal crossing. RON is
            // allowed to release and recreate either adjoining segment, so a saved
            // segment id can cease to be the id returned by the live node between
            // registry restoration and path execution. Re-resolve the complete
            // two-road join at the point where control is applied instead of
            // falling back to a connector-only path because of that transient id.
            RoadSnapResult liveJoin;
            return RoadSnapResolver.TryResolveSignalJoinNode(
        ...
        public static void UpdateSignalControllers(float simulationTimeDelta)
        {
            ...
            {
                BuiltSignalController controller = BuiltSignalControllers[i];
                bool cacheRefreshed = i == firstRefreshedIndex || i == secondRefreshedIndex;
                UpdateSignalController(ref controller, simulationTimeDelta, cacheRefreshed);
                BuiltSignalControllers[i] = controller;
            }

            _signalControllerSchedulerScans += (firstRefreshedIndex >= 0 ? 1 : 0)
                                              + (secondRefreshedIndex >= 0 ? 1 : 0);
            _signalControllerSchedulerDeferred += deferredScans;
            _signalControllerSchedulerLogElapsed += simulationTimeDelta;
            if (_signalControllerSchedulerLogElapsed >= SignalControllerSchedulerLogSeconds)
            {
                PedestrianCrossingLog.Advanced("[PedestrianCrossingToolkit] Signal controller scheduler: controllers="
                          + BuiltSignalControllers.Count
                          + " pedestrianScans="
                          + _signalControllerSchedulerScans
                          + " deferredDue="
                          + _signalControllerSchedulerDeferred
                          + " scanLimitPerUpdate="
                          + SignalPedestrianScansPerUpdate
                          + " reassertLimitPerSimulationFrame="
                          + SignalControllerReassertionsPerSimulationFrame);
                _signalControllerSchedulerLogElapsed = 0f;
                _signalControllerSchedulerScans = 0;
                _signalControllerSchedulerDeferred = 0;
            }
        }

        public static void ReapplySignalControllerStates()
        {
            int controllerCount = BuiltSignalControllers.Count;
            if (controllerCount == 0)
                return;

            if (_signalControllerReassertCursor < 0 || _signalControllerReassertCursor >= controllerCount)
                _signalControllerReassertCursor = 0;

            int reassertionCount = Math.Min(SignalControllerReassertionsPerSimulationFrame, controllerCount);
            for (int processed = 0; processed < reassertionCount; processed++)
            {
                int index = _signalControllerReassertCursor;
                BuiltSignalController controller = BuiltSignalControllers[index];
                if (controller.HasAppliedSignalState)
                    SetSignalControllerStates(ref controller, controller.AppliedVehicleState, controller.AppliedPedestrianState, false);
                BuiltSignalControllers[index] = controller;
                _signalControllerReassertCursor++;
                if (_signalControllerReassertCursor >= controllerCount)
                    _signalControllerReassertCursor = 0;
            }
        }
        ...
        {
            firstRefreshedIndex = -1;
            secondRefreshedIndex = -1;
            deferredScans = 0;
            int controllerCount = BuiltSignalControllers.Count;
            if (controllerCount == 0)
                return;
        ...
        {
            bool pedestriansOnCrossing = controller.HasPedestriansOnCrossing;
            switch (controller.Phase)
            {
                case SignalControllerPhase.Idle:
                    SetSignalControllerLights(ref controller, true);
                    if (!controller.HasPedestriansWaitingAtEntrance)
                    {
        ...
        private static void AddNearbyAlignedSignalRoadSegments(ref BuiltSignalController controller, Vector3 primaryDirection)
        {
            ...
            int minGridX = Mathf.Clamp(
                Mathf.FloorToInt((controller.Center.x - SignalNearbyRoadSegmentGridSearchRadius + SignalNetGridHalfExtent) / SignalNetGridCellSize),
                0,
                SignalNetGridResolution - 1);
            int maxGridX = Mathf.Clamp(
                Mathf.FloorToInt((controller.Center.x + SignalNearbyRoadSegmentGridSearchRadius + SignalNetGridHalfExtent) / SignalNetGridCellSize),
                0,
                SignalNetGridResolution - 1);
            int minGridZ = Mathf.Clamp(
                Mathf.FloorToInt((controller.Center.z - SignalNearbyRoadSegmentGridSearchRadius + SignalNetGridHalfExtent) / SignalNetGridCellSize),
                0,
                SignalNetGridResolution - 1);
            ...
            int traversalLimit = ManagerCapacity.GetExclusiveUpperBound(
                netManager.m_segments.m_size,
                segments.Length);
            for (int gridZ = minGridZ; gridZ <= maxGridZ; gridZ++)
            {
                int rowOffset = gridZ * SignalNetGridResolution;
                for (int gridX = minGridX; gridX <= maxGridX; gridX++)
                {
                    ushort segmentId = netManager.m_segmentGrid[rowOffset + gridX];
                    int traversed = 0;
                    while (segmentId != 0 && traversed++ < traversalLimit)
                    {
                        if (segmentId >= segments.Length)
                            break;

                        ref NetSegment segment = ref segments[segmentId];
                        ushort nextSegmentId = segment.m_nextGridSegment;
                        if ((segment.m_flags & NetSegment.Flags.Created) != 0 && HasVehicleLane(segment.Info))
                        {
                            Vector3 direction;
                            if (TryGetSegmentDirection(segmentId, out direction)
                                && Mathf.Abs(Vector3.Dot(primaryDirection, direction)) >= SignalControlledSegmentAlignmentDot)
                            {
        ...
        private static void RefreshSignalPedestrianCache(ref BuiltSignalController controller, bool force)
        {
            ...
            float crossingRadiusSqr = SignalPedestrianOnCrossingRadius * SignalPedestrianOnCrossingRadius;
            CitizenInstance[] buffer = citizenManager.m_instances.m_buffer;
            if (ScanSignalPedestriansInGrid(ref controller, citizenManager, buffer, nearRadiusSqr, waitingRadiusSqr, crossingRadiusSqr))
            {
        ...
        private static void AddBridgeConcrete(int assetId, Vector3 start, Vector3 end, float width, float startElevation, float endElevation, float thickness, string role)
        {
            ...
            AddBridgeGlassSides(assetId, raisedStart, raisedEnd, width, thickness, role);
            AddBridgeDeckFinishDetails(assetId, raisedStart, raisedEnd, width, thickness, role);
        }
        ...
        private static void AddBridgeDeckFinishDetails(int assetId, Vector3 raisedStart, Vector3 raisedEnd, float width, float deckThickness, string role)
        {
            if (role != "deck")
                return;
            ...
            float topOffset = deckThickness + BridgeDeckPanelJointHeight * 0.5f;
            float sideWarningOffset = Mathf.Max(0.02f, width * 0.5f - BridgeDeckSideWarningWidth * 0.5f - 0.08f);

            AddBridgeShellCube(
                "PCT Bridge Deck warning strip L #" + assetId,
                (raisedStart + raisedEnd) * 0.5f + side * sideWarningOffset + Vector3.up * topOffset,
                rotation,
                new Vector3(BridgeDeckSideWarningWidth, BridgeDeckPanelJointHeight, length),
                GetBridgeWarningStripeMaterial());
            AddBridgeShellCube(
                "PCT Bridge Deck warning strip R #" + assetId,
                (raisedStart + raisedEnd) * 0.5f - side * sideWarningOffset + Vector3.up * topOffset,
                rotation,
                new Vector3(BridgeDeckSideWarningWidth, BridgeDeckPanelJointHeight, length),
                GetBridgeWarningStripeMaterial());

            int joints = Mathf.FloorToInt(length / BridgeDeckPanelSpacing);
            for (int i = 1; i <= joints; i++)
            {
                float distance = i * BridgeDeckPanelSpacing;
                if (distance >= length - BridgeDeckPanelSpacing * 0.35f)
                    continue;

                AddBridgeShellCube(
                    "PCT Bridge Deck panel joint #" + assetId,
                    raisedStart + direction * distance + Vector3.up * topOffset,
                    rotation,
                    new Vector3(width + 0.08f, BridgeDeckPanelJointHeight, BridgeDeckPanelJointWidth),
                    GetBridgeTrimMaterial());
            }
        }
        ...
        private static void AddBridgeWindowRun(int assetId, Vector3 baseStart, Vector3 baseEnd, string role)
        {
            ...
            for (int i = 0; i < windows; i++)
            {
                ...
                AddBridgeWindowFrame(assetId, windowCenter, rotation, windowWidth, role);
            }
        }

        private static void AddBridgeWindowFrame(int assetId, Vector3 windowCenter, Quaternion rotation, float windowWidth, string role)
        {
            float frameLength = windowWidth + BridgeWindowFrameInset * 2f;
            float frameDepth = BridgeGlassThickness + BridgeWindowFrameInset;
            AddBridgeShellCube(
                "PCT Bridge Metal " + role + " window sill #" + assetId,
                windowCenter + Vector3.up * (BridgeWindowSillHeight + BridgeWindowFrameWidth * 0.5f),
                rotation,
                new Vector3(frameDepth, BridgeWindowFrameWidth, frameLength),
                GetBridgeTrimMaterial());
            AddBridgeShellCube(
                "PCT Bridge Metal " + role + " window header #" + assetId,
                windowCenter + Vector3.up * (BridgeWindowSillHeight + BridgeWindowHeight - BridgeWindowFrameWidth * 0.5f),
                rotation,
                new Vector3(frameDepth, BridgeWindowFrameWidth, frameLength),
                GetBridgeTrimMaterial());
        }
        ...
        private static void AddBridgePitchedRoofRun(int assetId, Vector3 raisedStart, Vector3 raisedEnd, float width, float deckThickness, string role)
        {
            Vector3 span = raisedEnd - raisedStart;
            Vector3 flatSpan = span;
            flatSpan.y = 0f;
            if (span.sqrMagnitude < 0.0001f || flatSpan.sqrMagnitude < 0.0001f)
                return;

            Vector3 side = new Vector3(-flatSpan.z, 0f, flatSpan.x).normalized;
            float halfWidth = (width + BridgeRoofOverhang * 2f) * 0.5f;
            float eaveLift = BridgeRoofThickness * 0.5f;
            float ridgeLift = BridgeRoofThickness + BridgeRoofPitchRise;
            Vector3 baseStart = raisedStart + Vector3.up * (deckThickness + BridgeWallHeight);
            Vector3 baseEnd = raisedEnd + Vector3.up * (deckThickness + BridgeWallHeight);

            GameObject roof = new GameObject("PCT Bridge Metal pitched roof " + role + " #" + assetId);
            Mesh mesh = new Mesh();
            mesh.vertices = new[]
            {
                baseStart - side * halfWidth + Vector3.up * eaveLift,
                baseStart + side * halfWidth + Vector3.up * eaveLift,
                baseStart + Vector3.up * ridgeLift,
                baseEnd - side * halfWidth + Vector3.up * eaveLift,
                baseEnd + side * halfWidth + Vector3.up * eaveLift,
                baseEnd + Vector3.up * ridgeLift
            };
            mesh.triangles = new[]
            {
                0, 3, 5, 0, 5, 2,
        ...
        private static void AddBridgeRoofRun(int assetId, Vector3 raisedStart, Vector3 raisedEnd, float width, float deckThickness, string role)
        {
            ...
            AddBridgePitchedRoofRun(assetId, start, end, width, deckThickness, role);
        }
        ...
        private static void AddBridgeAccessRoofRun(int assetId, Vector3 raisedStart, Vector3 raisedEnd, float width, float deckThickness, string role)
        {
            ...
            AddBridgePitchedRoofRun(assetId, raisedStart, raisedEnd, width, deckThickness, role);
        }
        ...
        private static Texture2D GetCrossingStripeTexture()
        {
            ...
            for (int y = 0; y < CrossingStripeTextureSize; y++)
            {
                for (int x = 0; x < CrossingStripeTextureSize; x++)
                {
                    ...
                    float lengthwiseScuff = Mathf.PerlinNoise(x * 0.065f + 72.4f, y * 0.18f + 9.2f);
                    float tyreWear = Mathf.PerlinNoise(x * 0.026f + 14.8f, y * 0.58f + 51.6f);
                    float fleck = Hash01((x + 17) * 73856093 ^ (y + 23) * 19349663);
                    float wear = Mathf.Lerp(0.78f, 1f, coarse) * Mathf.Lerp(0.88f, 1.04f, fine);
                    if (lengthwiseScuff > 0.72f)
                        wear *= Mathf.Lerp(0.82f, 0.96f, (lengthwiseScuff - 0.72f) / 0.28f);
                    if (tyreWear > 0.78f && Mathf.Abs(u - 0.5f) < 0.38f)
                        wear *= Mathf.Lerp(0.70f, 0.90f, (tyreWear - 0.78f) / 0.22f);
                    if (fleck < 0.10f)
                        wear *= Mathf.Lerp(0.62f, 0.84f, fleck / 0.10f);

                    float alpha = Mathf.Lerp(0.42f, 0.86f, edgeFade) * Mathf.Lerp(0.84f, 1f, coarse);
                    if (lengthwiseScuff > 0.75f)
                        alpha *= Mathf.Lerp(0.74f, 0.94f, (lengthwiseScuff - 0.75f) / 0.25f);
                    if (tyreWear > 0.82f && Mathf.Abs(u - 0.5f) < 0.35f)
                        alpha *= Mathf.Lerp(0.66f, 0.88f, (tyreWear - 0.82f) / 0.18f);
                    if (fleck < 0.07f)
        ...
        {
            return GetBridgeCosmeticMaterial(ref _bridgeWarningStripeMaterial, new Color(0.78f, 0.62f, 0.18f, 1f));
        }
        ...
        private static Material GetSubwayEntranceCanopyMaterial()
        {
        ...
        {
            if (_subwayEntranceTileMaterial != null)
                return _subwayEntranceTileMaterial;

            Shader shader = Shader.Find("Unlit/Color") ?? Shader.Find("Diffuse");
            if (shader == null)
                return GetSubwayEntranceWallMaterial();

            _subwayEntranceTileMaterial = new Material(shader);
            _subwayEntranceTileMaterial.color = new Color(0.82f, 0.78f, 0.64f, 1f);
            return _subwayEntranceTileMaterial;
        }

        private static Material GetSubwayEntranceSignMaterial()
        {
            if (_subwayEntranceSignMaterial != null)
                return _subwayEntranceSignMaterial;

            Shader shader = Shader.Find("Unlit/Color") ?? Shader.Find("Diffuse");
            if (shader == null)
                return GetSubwayStepNoseMaterial();

            _subwayEntranceSignMaterial = new Material(shader);
            _subwayEntranceSignMaterial.color = new Color(0.08f, 0.24f, 0.32f, 1f);
            return _subwayEntranceSignMaterial;
        }
